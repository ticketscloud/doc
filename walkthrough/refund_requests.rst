...
+-------------------------------------------------------------------------+--------------------------------------+
| Reason is unavailable now.                                              | по указанной причине возврат не      |
|                                                                         | осуществляется                       |
+-------------------------------------------------------------------------+--------------------------------------+
| No reason nor lower_minimal_days setting                                | запрещено для возврата < 3 дней      |
|                                                                         | до начала мероприятия                |
+-------------------------------------------------------------------------+--------------------------------------+
| Reason required for discounted tickets: No discounted.percentage        | нет настройки для возврата           |
| setting found                                                           | билетов со скидкой                   |
+-------------------------------------------------------------------------+--------------------------------------+
| Operation failed: Event started, and no after_event_start setting.      | запрещено для возврата когда         |
...
+-------------------------------------------------------------------------+--------------------------------------+
| Problem with customer in vibe: [<ticket_id>, ...]                       | указанные билеты проданы/подарены в  |
|                                                                         | приложении VIBE другому человеку     |
+-------------------------------------------------------------------------+--------------------------------------+
| Problem with check customer in vibe, please retry                       | возникла проблема с проверкой        |
|                                                                         | владельца билета, повторите позже    |
+-------------------------------------------------------------------------+--------------------------------------+
...
|                                                                         | возврат невозможен                   |
+-------------------------------------------------------------------------+--------------------------------------+
| Category {category_id} is not fz-193                                    | категория билетов запрещена к        |
|                                                                         | возврату по ФЗ-193                   |
+-------------------------------------------------------------------------+--------------------------------------+
...
+----------------------------------------------------------------+--------------------------------------+
| Only user culprit allowed for non cancelled events             | Не используется в связи с доработкой |
|                                                                | логики для перенесённых мероприятий. |
| NB: больше не используется, т.к. организатор может также       | См. следующее сообщение ↓↓↓          |
| подтвердить возврат и по перенесённому мероприятию             |                                      |
+----------------------------------------------------------------+--------------------------------------+
| Only user culprit allowed for non cancelled not postponed      | только пользователь может подтвердить|
| events                                                         | возврат для не отмененного           |
|                                                                | и не перенесённого мероприятия       |
+----------------------------------------------------------------+--------------------------------------+
